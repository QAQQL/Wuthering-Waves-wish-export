        ...
        <el-dropdown @command="optionCommand">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              ...
              <el-dropdown-item :disabled="!allowClick() || state.status === 'loading'" command="proxy" icon="position">
    ...
    <el-dialog :title="ui.button.solution" v-model="state.showCacheCleanDlg" width="90%" custom-class="max-w-md">
      ...
      <template #footer>
        <div class="dialog-footer text-center">
          <el-button type="primary" @click="state.showCacheCleanDlg = false" class="focus:outline-none">
            {{ ui.common.ok }}
          </el-button>
        </div>
      </template>
...
const detail = computed(() => {
  let data = state.dataMap.get(state.current)
  if (data) {
    data = gachaDetail(data.result)
